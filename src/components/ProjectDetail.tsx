...
function IssueItem({ issue, defaultOpen = false }: { issue: Issue; defaultOpen?: boolean }) {
  ...
  return (
    <li>
      <button
        className={`flex w-full items-center gap-2.5 rounded-md py-1 text-left text-sm transition-colors ${hasDetail ? 'cursor-pointer hover:bg-neutral-50' : 'cursor-default'}`}
      ...
      >
        ...
        <span className={`flex-1 ${issue.status === 'closed' ? 'text-neutral-400' : 'text-neutral-700'}`}>
          {issue.title}
        </span>
        {hasDetail && (
          <svg
            className={`h-3.5 w-3.5 shrink-0 text-neutral-300 transition-transform ${open ? 'rotate-180' : ''}`}
      ...
      {hasDetail && open && (
        <div className="mb-2 ml-6 mt-1 space-y-2 rounded-md border border-neutral-100 bg-neutral-50 p-3">
          {issue.problem && (
            <div>
              <span className="text-xs font-semibold text-red-400">문제</span>
              <p className="mt-0.5 text-xs leading-relaxed text-neutral-600">{issue.problem}</p>
            </div>
          )}
          {issue.solution && (
            <div>
              <span className="text-xs font-semibold text-blue-400">해결</span>
              <p className="mt-0.5 text-xs leading-relaxed text-neutral-600">{issue.solution}</p>
            </div>
          )}
        </div>
      )}
          ...
          <div className="mt-8">
            ...
            <ul className="space-y-0.5">
              {project.issues.map((issue, i) => (
                <IssueItem key={`${project.id}-${i}`} issue={issue} defaultOpen={i === 0} />
              ))}
            </ul>
          </div>
        )}
